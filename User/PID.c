#include "stm32f10x.h"                  // Device header
#include "BlueTooth.h"
#include "PID.h"  
#include "Task_Init.h"

float PID_Parameter[4];
uint8_t signedflag = 0;

void PID_Init(PID_t *p)
{
	p->ErrorInt = 0;
	p->Error0 = 0;
	p->Error1 = 0;
	p->Out = 0;
	p->Actual = 0;
	p->Actual2 = 0;
	p->Target = 0;
}

void PID_Updata(PID_t *p)
{
	p->Error1 = p->Error0;
	p->Error0 = (p->Target - p->Actual);
	
	if(p->Ki == 0)p->ErrorInt = 0;
	else p->ErrorInt += p->Error0;
	
	if(p->ErrorInt > 3200)p->ErrorInt = 3200;
	if(p->ErrorInt < -3200)p->ErrorInt = -3200;
	
	// p->Diout = p->Ki * p->ErrorInt;
	
	// p->Out = p->Kp * p->Error0 + p->Diout - p->Kd * (p->Actual - p->Actual1);
	p->Out = p->Kp * p->Error0 + p->Ki * p->ErrorInt + p->Kd * (p->Error0 - p->Error1);
	
	if(p->Out > 0)p->Out += p->OutOffset;
	if(p->Out < 0)p->Out -= p->OutOffset;

	if(p->Out > p->OutMax)p->Out = p->OutMax;
	if(p->Out < p->OutMin)p->Out = p->OutMin;
	
	// p->Actual1 = p->Actual;
	
	// p->Error1 = p->Error0;
	// p->Error0 = (p->Target - p->Actual);
	
	// if(p->Ki == 0)p->ErrorInt = 0;
	// else p->ErrorInt += p->Error0;
	
	// if(p->ErrorInt > 3200)p->ErrorInt = 3200;
	// if(p->ErrorInt < -3200)p->ErrorInt = -3200;
	
	// p->Out = p->Kp * p->Error0 + p->Ki * p->ErrorInt + p->Kd * (p->Error0 - p->Error1);
	
	// if(p->Out > p->OutMax)p->Out = p->OutMax;
	// if(p->Out < p->OutMin)p->Out = p->OutMin;
}


//void PID_SetParameter_v2(PID_t *pid)
//{
//	if(BlueTooth_GetFlagStatus() == 1)
//	{
//		char *p = strtok(BlueTooth_RxPackage,",");
//		if(strcmp(p,"s") == 0)
//		{
//			char *Name = strtok(NULL,",");
//			char *Value = strtok(NULL,",");
//			if(strcmp(Name,"Kp") == 0)
//			{
//				pid->Kp = atof(Value);
//			}
//			if(strcmp(Name,"Ki") == 0)
//			{
//				pid->Ki = atof(Value);
//			}
//			if(strcmp(Name,"Kd") == 0)
//			{
//				pid->Kd = atof(Value);
//			}
//		}
//		else if(strcmp(p,"j") == 0)
//		{
//		}
//	}
//}

